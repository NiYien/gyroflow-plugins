    ...
    def test_both_debian_source_layouts_keep_signatures_and_are_repeatable(self):
        for layout in ("sources.list", "sources.list.d/debian.sources"):
            with self.subTest(layout=layout), tempfile.TemporaryDirectory() as directory:
                ...
                self.assertEqual(sources.count("check-valid-until=no"), 3)
                self.assertTrue(all("http://snapshot.debian.org/archive/" in line for line in lines))
                self.assertTrue(all("/20260831T000000Z/" in line for line in lines))
                self.assertIn("/debian-security/20260831T000000Z/ bullseye-security main", sources)
