...
- **Host environment findings** (recorded for future sessions): ~~Resolve 21.0.0.47 timeline-settings-dialog edits do not refresh the scripting-API snapshot store (GetSetting stale until project load / API SetSetting / toggling "Use Project Settings" off-on — the user-level refresh recipe)~~ — **RETRACTED, see the 2026-07-26 note below**; Resolve Edit-page `Fit` can expose either source-native squeezed or PAR-composited timeline buffers depending on clip/timeline state, so neither model is a global invariant; Resolve's FillCrop input-sizing decision uses physical pixel dims (confirmed live via R5MK2).
- **2026-07-26 retraction of the snapshot-staleness finding** (change `openfx-mismatch-mode-refresh`): re-verified live on the same host build (21.0.0.47). The single-key `GetSetting('key')` form refreshes **immediately** on dialog save, at both project and timeline level — known start `scaleToCrop`, user selects Center crop, the read returns `centerCrop`, and a 314-key dump diff moves exactly that one key. The original spike searched the **no-argument** `tl:GetSetting()` dump, which on a timeline returns the project-level value set (or, once custom settings are on, only the overridden subset) and therefore cannot contain the override; the "no live read path" inference broke at its first link. The user-visible "setting change does not apply until Resolve restarts" symptom was entirely plugin-side: the render path never re-queried, and the `ReloadProject` button that §8.2 relies on for refresh is `hidden: true`. One real but separate host quirk survives: the snapshot can carry cross-session residue until the first dialog save of a session (observed: a 6-day-old timeline resolution surviving a restart and a project open), which periodic re-reading dissolves.
